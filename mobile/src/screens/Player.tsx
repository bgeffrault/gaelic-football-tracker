...
const usePlayerShoots = (playerId: number) => {
  ...
  return {
    pointCount: playerShoots?.filter((shoot) => shoot.type === "point")?.[0]
      ?.count,
    goalCount: playerShoots?.filter((shoot) => shoot.type === "goal")?.[0]
      ?.count,
    missedCount: playerShoots?.filter((shoot) => shoot.type === "missed")?.[0]
      ?.count,
    blockedCount: playerShoots?.filter((shoot) => shoot.type === "blocked")?.[0]
      ?.count,
    playerShoots,
  };
};
...
export function Player({ navigation, route }: AppNavigationProp<"Player">) {
  ...
  const { data: player, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await supabaseClient
        .from("Members")
        .select("id, firstName, lastName, pseudo, license")
        .eq("id", playerId)
        .single();
      return result.data;
    },
  ...
  const { pointCount, goalCount, blockedCount, missedCount, playerShoots } =
    usePlayerShoots(playerId);
  const accuracy =
    ((goalCount + pointCount) /
      (goalCount + missedCount + pointCount + blockedCount)) *
    100;
  const accuracyString = Number.isNaN(accuracy) ? "" : accuracy.toFixed(2);
